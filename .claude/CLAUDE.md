...
# Project: seungah-portfolio

## Architecture

- **Hybrid 2-Tab**: SSR Portfolio (`/`) + Client Chat tab, controlled by TabContext
- **Framework**: Next.js 15 App Router, React 19, TypeScript 5, Tailwind CSS 4
- **RootLayout**: Server Component (no 'use client'). Client logic in ClientProviders.
- **Tab System**: TabNavigation (WAI-ARIA) + TabContentWrapper (CSS show/hide)
- **Dark Mode**: next-themes (attribute="class", FOUC-free)
- **i18n**: Dual pattern — server `t(locale, key)` + client `useT()` hook
- **AI Chat**: Gemini API via `/api/chat` with streaming, rate limiting (10 req/min), conversation bounds (20 messages)
- **A2UI**: Client-side keyword detection attaches data cards below AI responses
- **useChat Hook**: Extracted chat state/streaming logic in `src/hooks/useChat.ts`
- **Contact**: Nodemailer via `/api/contact` with rate limiting (3 req/min), input sanitization

## Key Conventions

- Server Components by default; `'use client'` only for interactive boundaries
- All user-facing strings via i18n (ko.json / en.json)
- Apple-style design: neutral palette, rounded corners, subtle shadows, generous whitespace
- Tailwind `dark:` variants for dark mode (no manual .dark CSS overrides)
- Next.js `<Image>` for all images with proper alt text
- Semantic HTML + ARIA attributes per .claude/skills/ guidelines
- WAI-ARIA Tabs pattern for tab navigation
...
- `src/components/chat/` — AI Chat tab (ChatTab, ChatInput, ChatMessage, A2UICards, cards/)
- `src/components/layout/` — TabNavigation, TabContentWrapper, ThemeToggle, LanguageToggle
- `src/components/portfolio/` — PortfolioContent, AboutSection, CareerSection, ProjectSection, ContactSection
- `src/components/providers/` — ClientProviders (Theme + I18n + Tab)
- `src/components/ui/` — Reusable UI (SkillBadge)
- `src/helpers/datas/` — Static data (career, profile, projects)
- `src/hooks/` — Custom hooks (useChat)
- `src/lib/chat/` — Gemini client, system prompt, keyword detector, markdown
- `src/lib/i18n/` — Translations (ko/en), t(), useT()
...
- `pnpm lint` — ESLint
- Pre-commit hook: husky + lint-staged (ESLint on staged .ts/.tsx files)
