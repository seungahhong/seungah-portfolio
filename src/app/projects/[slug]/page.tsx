...
const projectDetails = {
  portfolio: {
    ...
    description: "Next.js, Tailwind CSS로 만든 개발자 포트폴리오. 반응형, 다크모드, SEO 최적화까지 모두 구현.",
  ...
  gallery: {
    ...
    description: "반응형 이미지 그리드와 라이트박스 UI, 이미지 업로드 및 다운로드 지원.",
...
export default async function ProjectDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <section className="max-w-2xl mx-auto">
      ...
      <div className="mb-6">
        <h2 className="text-xl font-semibold mb-2 text-gray-900 dark:text-white">Tech Stack</h2>
        <ul className="flex flex-wrap gap-2">
          {project.tech.map((t) => (
            <li key={t} className="bg-indigo-100 dark:bg-indigo-800/40 text-indigo-700 dark:text-indigo-200 px-3 py-1 rounded-full text-sm font-medium">{t}</li>
          ))}
        </ul>
      </div>
      <a
